...
class GMailProviderBot(BaseProviderBot):
    ...
    async def start(self, user_data: any):        
        try:
            ...
            if self.sync_time == -1:
                last_message = bridge.get_last_message(provider_name=PROVIDER_NAME, 
                                                    access_token=self.access_token, 
                                                    option="")
                ai_response = ai_service.get_response(service_name="", message=last_message['snippet'], option="")
                bridge.reply_to_message(provider_name=PROVIDER_NAME,
                                        access_token=self.access_token,
                                        to=last_message['messageId'],
                                        message=ai_response['message'],
                                        option="")                
                print(f"LastMessage: {last_message['snippet']}, response: {ai_response['message']}")
            else:
                last_messages = bridge.get_messages(provider_name=PROVIDER_NAME,
                                                    access_token=self.access_token,
                ...
                for message in last_messages['messages']:
                    ai_response = ai_service.get_response(service_name="", message=message['snippet'], option="")
                    bridge.reply_to_message(provider_name=PROVIDER_NAME,
                                            access_token=self.access_token,
                                            to=message['messageId'],
                                            message=ai_response['message'],
                                            option="")                
                    print(f"Message: {message['snippet']}, response: {ai_response['message']}")
            ...
            self.access_token = bridge.get_access_token_from_refresh_token(provider_name=PROVIDER_NAME, 
